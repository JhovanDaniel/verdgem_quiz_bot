class LeaderboardController < ApplicationController
  ...
  def index
    ...
    # Get top users by total score for the current month (minimum 5 questions attempted)
    @top_by_score = User.joins(:question_attempts)
                        .where(role: 'student')
                        .where(question_attempts: { created_at: @start_date..@end_date })
                        .group('users.id')
                        .having('COUNT(question_attempts.id) >= 5')
                        .select('users.*, SUM(question_attempts.score) as total_points')
                        .order('total_points DESC')
                        .limit(10)
    
    # Get top users by percentage for the current month (minimum 5 questions attempted)
    @top_by_percentage = User.where(role: 'student')
                             .select { |user| user.completed_questions_count(@start_date, @end_date) >= 5 }
                             .sort_by { |user| -user.score_percentage(@start_date, @end_date) }
                             .take(10)
    
    # Get current user's rank for each category
    if current_user
      if current_user.completed_questions_count(@start_date, @end_date) >= 5
